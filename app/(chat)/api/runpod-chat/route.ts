...
export async function POST(request: Request) {
  ...
  console.log(
    `[${requestId}] RunPod chat request started at ${requestStart.toISOString()}`
  );
  ...
  try {
    ...
    console.log(
      `[${requestId}] Using Gemma model, temperature: ${temperature}, topP: ${topP}, maxTokens: ${maxTokens}`
    );
    console.log(
    ...
    console.log(`[${requestId}] Starting Gemma response stream`);
    return createDataStreamResponse({
      execute: (dataStream) => {
        ...
        // Use gemma provider with the OpenAI-compatible interface
        // We'll configure the provider to include our request ID for tracing
        const gemmaProvider = runpod;

        // Add request ID to first message for tracing in the backend
        const augmentedMessages = [...messages];
        if (
          augmentedMessages.length > 0 &&
          augmentedMessages[0].role === "user"
        ) {
          augmentedMessages[0].content = `[RequestID: ${requestId}] ${augmentedMessages[0].content}`;
        }

        const result = streamText({
          model: gemmaProvider("gemma-7b"),
          system: systemPrompt({ selectedChatModel: "gemma-7b" }),
      ...
      onError: (error) => {
        console.error(`[${requestId}] Error in Gemma stream:`, error);
        return "Oops, an error occurred with Gemma processing!";
      },
    });
  } catch (error) {
    ...
    return new Response(
      "An error occurred while processing your Gemma request!",
      {
        status: 404,
      }
    );
